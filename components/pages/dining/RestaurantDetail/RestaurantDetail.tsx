...
export const RestaurantDetail: React.FC<IDiningOrdersProps> = ({
  ...
  const [getSlots, { loading: tableLoading }] = useMutation(GET_SLOT_DETAILS, {
    ...
    onError(error) {
      ...
      const slotFilled = errorCode.some(
        (e: any) =>
          e.message.toLowerCase().includes('no availability for the number of seats') &&
          e?.data == null,
      );
      if (slotFilled) {
        ...
        notificationStorage({
          ...
          description: t(
            'The selected number of seats is not available. Please adjust your selection or choose another time.',
          ),
          redirect: null,
          type: FAILURE,
        });
