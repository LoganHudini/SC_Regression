...
test.describe('Sidecar + StaffConnect Flow', () => {
  ...
  test.skip('Complete Sidecar creation flow', async () => {

    test.setTimeout(120000);

    // Launch Sidecar
    await test.step(
      'Verify that the user is navigated to the Sidecar login page on launching the URL, and the Email ID field is present and allows input, with the Continue button visible and clickable',
      async () => {
    ...
    // Login
    await test.step(
      'Verify that the user can enter their password and click the login button',
      async () => {
    ...
    // Select Property
    await test.step(
      'Verify that the user can select a property from the dropdown',
      async () => {
    ...
    // Verify Login
    await test.step(
      'Verify that the user is successfully logged in and navigated to the Sidecar main page',
      async () => {
        // Connect Device
  ...
  test('Complete StaffConnect flow', async ({ browser }) => {
    ...
    await test.step(
      'Verify that the user is navigated to the StaffConnect login page on launching the URL, and the Email ID field is present and allows input, with the Continue button visible and clickable',
      async () => {
    ...
    await test.step(
      'Verify that the user is successfully logged in and navigated to the StaffConnect main page',
      async () => {
        await staffConnect.verifySuccessfulLogin();
      });
    // await test.step.skip(
    //   'Verify that the sidecar connection created in the previous session is visible in StaffConnect',
    //   async () => {
    ...
    //   });
    await test.step(
      'Verify that the arrival section headers details are visible in the guests module in StaffConnect',
      async () => {
        await staffConnect.verifyArrivalSection();
      });
    await test.step(
      'Verify that the user can search for a guest using the confirmation number in the guests module in StaffConnect',
      async () => {
        await staffConnect.verifyArrivalGuests(CONFIRMATION_NUMBER);
      });
    await test.step(
      'Verify that the user can view the resrvation details and arrival information in StaffConnect',
      async () => {
        await staffConnect.verifyDetailsinReservationSection();
      });
await test.step(
    'Verify that the user able add the reservation note and able to click on AddNote button in StaffConnect',
    async () => {
      await staffConnect.verifyAddingNotesinReservationSection(RESERVATION_NOTE);
    });
    await test.step(
      'verify that the user can navigate to the Guest Tab and verify the details in Guest Tab in StaffConnect',
      async () => {
